          ...
          <div className="flex items-center justify-between h-42.5">
            ...
            <Link to="/" className="flex items-center cursor-pointer group shrink-0">
              ...
              <div className="hidden sm:block">
                ...
                <span className="text-[#DD9C00] text-[10px] font-black tracking-[0.2em] block group-hover:text-yellow-400 transition-colors uppercase">Engineering</span>
              </div>
            </Link>
                ...
                <>
                  {navItems.slice(0, 2).map((item) => (
                    <Link
                      key={item.label}
                      to={item.path}
                      className={`flex flex-col items-center justify-center h-16 px-4 gap-1 group transition-all rounded-xl ${isActive(item.path) ? 'bg-slate-900 border-b-2 border-[#DD9C00]' : 'hover:bg-slate-900/50'}`}
                    >
                      <item.icon size={24} className={`transition-colors ${isActive(item.path) ? 'text-[#DD9C00]' : 'text-slate-400 group-hover:text-[#DD9C00]'}`} />
                      <span className={`text-xs font-black uppercase tracking-widest ${isActive(item.path) ? 'text-[#DD9C00]' : 'text-slate-400 group-hover:text-slate-200'}`}>{item.label}</span>
                    </Link>
                  ))}
                  ...
                  >
                    <button className={`flex flex-col items-center justify-center h-16 px-4 gap-1 rounded-xl transition-all group-hover:bg-slate-900/50 ${isMarketActive ? 'bg-slate-900 border-b-2 border-[#DD9C00]' : ''}`}>
                        <ShoppingCart size={24} className={`transition-colors ${isMarketActive ? 'text-[#DD9C00]' : 'text-slate-400 group-hover:text-[#DD9C00]'}`}/>
                        <span className={`text-xs font-black uppercase tracking-widest transition-colors ${isMarketActive ? 'text-[#DD9C00]' : 'text-slate-400 group-hover:text-slate-200'}`}>
                      ...
                      <div className="absolute top-full left-0 mt-1 w-60 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-2 z-[60] animate-in fade-in slide-in-from-top-2">
                        <div className="grid grid-cols-1 gap-1">
                          {marketDropdownItems.map((item) => (
                            ...
                            >
                              <div className="p-2 rounded-lg bg-slate-950 border border-slate-800 group-hover/item:border-[#DD9C00]/50 transition-colors">
                                <item.icon size={20} className="text-slate-400 group-hover/item:text-[#DD9C00] transition-colors"/>
                              </div>
        ...
        {isOpen && (
          <div className="lg:hidden bg-slate-950 border-b border-slate-800 animate-in slide-in-from-top-5 max-h-[calc(100vh-6rem)] overflow-y-auto">
            <div className="px-4 pt-4 pb-8 space-y-2">
              ...
              {/* Only show standard links to buyers/guests */}
              {!isStaff && (
                <>
                  {navItems.map((item) => (
                    <Link
                      ...
                      className={`flex items-center gap-4 px-5 py-4 rounded-xl text-base font-black uppercase tracking-widest transition-all ${isActive(item.path) ? 'text-[#DD9C00] bg-slate-900 border-l-4 border-[#DD9C00] shadow-inner' : 'text-slate-400'}`}
                    >
                      <item.icon size={22} className={isActive(item.path) ? 'text-[#DD9C00]' : 'text-slate-400'} />
                      {item.label}
                    </Link>
                  ))}
                  ...
                  <div className="border-t border-slate-800 pt-4 mt-4">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {marketDropdownItems.map(item => (
                          <Link key={item.label} to={item.path} onClick={() => setIsOpen(false)} className="flex items-center gap-4 px-8 py-3 rounded-xl text-sm font-black text-slate-400 hover:text-[#DD9C00] hover:bg-slate-900 transition-all uppercase tracking-widest group">
                            <item.icon size={20} className="text-slate-500 group-hover:text-[#DD9C00] transition-colors"/>
                            {item.label}
                          </Link>
                        ))}
                    </div>
               ...
               <Link to="/erp" onClick={() => setIsOpen(false)} className="text-[#DD9C00] font-black uppercase tracking-[0.2em] block px-5 py-5 rounded-xl text-center mt-6 border border-[#DD9C00]/20 bg-[#DD9C00]/5 shadow-inner">
               ...
               <div className="pt-6 border-t border-slate-800 mt-6 px-2">
                {user ? (
                    <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800">
                      <div className="text-slate-500 text-[11px] font-black uppercase tracking-widest mb-3">{isStaff ? 'Admin System' : 'Buyer Account'}</div>
                      <div className="flex flex-col gap-3">
                        
                        {isStaff ? (
                          <Link to="/admin" onClick={() => setIsOpen(false)} className="block py-3 px-4 bg-slate-950 rounded-lg text-slate-200 font-black uppercase tracking-widest text-xs border border-slate-800 text-center">Control Panel</Link>
                        ) : (
                          <Link to="/buyer/dashboard" onClick={() => setIsOpen(false)} className="block py-3 px-4 bg-slate-950 rounded-lg text-slate-200 font-black uppercase tracking-widest text-xs border border-slate-800 text-center">Dashboard</Link>
                        )}
                    ...
                    <button onClick={() => { setShowAuthModal(true); setIsOpen(false); }} className="w-full text-center bg-[#DD9C00] text-slate-900 py-5 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl hover:brightness-110 transition-all">ACCOUNT</button>
                )}
               </div>
            </div>
          </div>
